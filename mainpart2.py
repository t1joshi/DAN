...
from SUBWORDDANmodels import SentimentDatasetDANBPE, NN2DAN, NN3DAN, train_bpe_tokenizer
...
def train_epoch(data_loader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(data_loader):
        X = X.long() #changed from float to long
...
def main():
    vocab_size = 30000
    embedding_dim = 300
    sentences = [example.words for example in read_sentiment_examples("data/train.txt")]
    tokenizer = train_bpe_tokenizer(sentences, vocab_size, min_frequency=2)
    ...
    if args.model == "BOW":
        ...
        print('\n2 layers:')
        nn2_train_accuracy, nn2_test_accuracy = experiment(NN2DAN(vocab_size, embedding_dim, hidden_size=100), train_loader, test_loader)


        print('\n3 layers:')
        nn3_train_accuracy, nn3_test_accuracy = experiment(NN3DAN(vocab_size, embedding_dim, hidden_size=250), train_loader, test_loader)
